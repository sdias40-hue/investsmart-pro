...
# 2. Motores de Inteligência e Notícias
@st.cache_data(ttl=300)
def buscar_noticias_v300(t):
    try:
        tk = yf.Ticker(t)
        return tk.news[:3]
    except: return []
...
def buscar_v300(t):
    try:
        ...
        # Filtro de Dados (Fim do erro image_41bcc2.jpg no BTC)
...
with st.sidebar:
    ...
    tkn = st.text_input("Token Telegram:", type="password", key="tk_v300")
    cid = st.text_input("Seu ID:", value="8392660003", key="id_v300")
    
    st.divider()
    st.subheader("🚀 Análise e Trade")
    
    with st.form("form_v300", clear_on_submit=True):
...
if st.session_state.consulta:
    d = buscar_v300(st.session_state.consulta)
    if d:
        pa = d['h']['Close'].iloc[-1]
        ...
        if not d['is_c']:
            ...
            st.info(f"🤖 **Mentor:** Setor: {d['info'].get('sector', 'N/A')} | P/L: {d['info'].get('forwardPE', 0):,.1f}")
        else:
        ...
        for n in buscar_noticias_v300(st.session_state.consulta):
...
# SEÇÃO 2: MONITORAMENTO ATIVO (Sinais image_242d00.png)
if st.session_state.radar:
    for t_at, cfg in list(st.session_state.radar.items()):
        d_at = buscar_v300(t_at)
        if d_at:
            ...
            with st.expander(f"📈 VIGIANDO: {t_at}", expanded=True):
                ...
                with col3:
                    ...
                    if p_now <= sup * 1.015: st.success("🔥 SINAL: Ponto de Pullback no Suporte!")
                    elif p_now >= res * 0.985: st.error("⚠️ SINAL: Ponto de Breakout na Resistência!")
                    else: st.write(f"⚖️ Neutro. Alvo: {cfg['alvo']:,.2f}")
