...
# 1. Setup Visual High Clarity (Fundo Claro - image_df2bc5.jpg)
st.set_page_config(page_title="InvestSmart Pro | Elite", layout="wide")
st.markdown("<style>.main { background-color: #f8f9fa; }</style>", unsafe_allow_html=True)

# 2. Motor de Mensagem e Busca Global
...
def buscar_v96(t):
    try:
        ...
        # Busca Global para Criptos (US$) e B3 para Ações (R$)
...
with st.sidebar:
    ...
    with col1:
        if st.button("🚀 Monitorar"):
            if t_in:
                st.session_state.monitor_v96[t_in] = {
                    "is_c": is_c_in, "v_brl": v_inv, "p_in": p_comp, "alvo": p_alvo, "qtd": qtd
                }
                st.rerun()
    with col2:
        if st.button("🧹 Limpar Campos"):
            # Limpa apenas os inputs, mantendo o monitoramento (image_2615ba.png)
    ...
    for t_list in list(st.session_state.monitor_v96.keys()):
        st.write(f"• {t_list}")

# --- PAINEL PRINCIPAL ---
st.title("🏛️ InvestSmart Pro | Monitoramento em Tempo Real")

if not st.session_state.monitor_v96:
    st.info("Sistema Online. Adicione ativos na lateral para iniciar.")
else:
    for t_mon in list(st.session_state.monitor_v96.keys()):
        cfg = st.session_state.monitor_v96[t_mon]
        h, info, dolar, is_crypto = buscar_v96(t_mon)
        ...
        if h is not None and not h.empty:
            ...
            with st.expander(f"📊 MONITORANDO: {t_mon}", expanded=True):
                ...
                with c1:
                    ...
                    if st.button(f"🗑️ Parar {t_mon}", key=f"btn_{t_mon}"):
                        del st.session_state.monitor_v96[t_mon]
                        st.rerun()

                with c2:
                    st.metric("Lucro (R$)", f"R$ {lucro_brl:,.2f}", f"{((p_agora/cfg['p_in'])-1)*100:.2f}%" if cfg['p_in'] > 0 else "0%")
                    st.caption(f"Valor Total: R$ {v_total_brl:,.2f}")
                
                with c3:
                    st.subheader("🤖 Mentor Estratégico")
                    if cfg['alvo'] > 0:
                        v_no_alvo = total_u * (cfg['alvo'] * (dolar if is_crypto else 1))
                        lucro_alvo = v_no_alvo - v_invest_brl
                ...
                st.plotly_chart(fig, use_container_width=True, key=f"v96_{t_mon}")
        st.divider()
